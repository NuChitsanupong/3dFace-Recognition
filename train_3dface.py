...
def train_model(train_dataset, eval_dataset, input_channels, num_of_classes,
                num_of_pretrained_classes,
                num_of_epochs, batch_size, num_of_workers,
                log_base_dir, pretrained_on_imagenet=True,
                is_resnet50=False, is_resnext50=False, is_mobilenet=False, 
                is_vgg16=False, is_predicted=False,
    ...
    Args:
        ...
        :param num_of_pretrained_classes: (int)
        :param is_ResNet50: (bool)
        :param is_resnext50: (bool)
        :param is_mobilenet: (bool)
        :param is_vgg16: (bool)
    ...
    if is_resnet50:
        if is_predicted:
            ...
            model = ResNet50(input_channels, num_of_pretrained_classes,  pretrained=True)
            model.load_state_dict(checkpoint)
            model.fc = nn.Linear(model.fc.in_features, num_of_classes)
            print("Model train with ResNet50: load from " + pretrained_model_path)
        else:
            model = ResNet50(input_channels, num_of_classes, pretrained=pretrained_on_imagenet)
            print("Model train with ResNet50")
    elif is_resnext50:
        if is_predicted:
            checkpoint = torch.load(pretrained_model_path)
            model = resnext50(input_channels, num_of_pretrained_classes,  pretrained=True)
            model.load_state_dict(checkpoint)
            model.fc = nn.Linear(model.fc.in_features, num_of_classes)
            print("Model train with resNext50: load from " + pretrained_model_path)
        else:
            model = resnext50(input_channels, num_of_classes, pretrained=pretrained_on_imagenet)
            print("Model train with resNext50")
    elif is_mobilenet:
        if is_predicted:
            checkpoint = torch.load(pretrained_model_path)
            model = mobilenet(input_channels, num_of_pretrained_classes,  pretrained=True)
            model.load_state_dict(checkpoint)
            model.classifier[1] = nn.Linear(model.classifier[1].in_features, num_of_classes)
            print("Model train with mobileNet_v2: load from " + pretrained_model_path)
        else:
            model = mobilenet(input_channels, num_of_classes, pretrained=pretrained_on_imagenet)
            print("Model train with mobileNet_v2")
    elif is_vgg16:
        if is_predicted:
            checkpoint = torch.load(pretrained_model_path)
            model = vgg16(input_channels, num_of_pretrained_classes,  pretrained=True)
            model.load_state_dict(checkpoint)
            model.classifier[6] = nn.Linear(model.classifier[6].in_features, num_of_classes) 
            print("Model train with vgg16: load from " + pretrained_model_path)
        else:
            model = vgg16(input_channels, num_of_classes, pretrained=pretrained_on_imagenet)
            print("Model train with vgg16")
...
#        optimizer.load_state_dict(torch.load(pretrained_optim_path, map_location=device))
    ...
    best_accu = 0.0
    saved_model_path = os.path.join(log_base_dir, '3dface-model.pkl')
    saved_optim_path = os.path.join(log_base_dir, '3dface-optim.pkl')
...
def parse_arguments(argv):
    ...
    parser.add_argument('--num_of_pretrained_classes', type=int,
                        help='Number of channels of the last output layer.', default=1200)
    parser.add_argument('--is_resnet50', type=bool,
                        help='Train model with ResNet50.', default=False)
    parser.add_argument('--is_resnext50', type=bool,
                        help='Train model with resNext50.', default=False)
    parser.add_argument('--is_mobilenet', type=bool,
                        help='Train model with mobileNet_v2.', default=False)
    parser.add_argument('--is_vgg16', type=bool,
                        help='Train model with vgg16.', default=False)
    parser.add_argument('--is_predicted', type=bool,
                        help='Model for predicted.', default=False)
...
if __name__ == '__main__':
    ...
    model = train_model(train_dataset, eval_dataset,
                        ...
                        num_of_pretrained_classes=args.num_of_pretrained_classes,
                        ...
                        pretrained_on_imagenet=True, is_resnet50=args.is_resnet50, 
                        is_resnext50=args.is_resnext50, is_mobilenet=args.is_mobilenet,
                        is_vgg16=args.is_vgg16, is_predicted=args.is_predicted,
